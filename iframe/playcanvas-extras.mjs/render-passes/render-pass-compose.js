...
class RenderPassCompose extends RenderPassShaderQuad {
  ...
  frameUpdate() {
    if (this._shaderDirty) {
      ...
      const key = `${this.toneMapping}` + `-${this.bloomTexture ? 'bloom' : 'nobloom'}` + `-${this.gradingEnabled ? 'grading' : 'nograding'}` + `-${this.vignetteEnabled ? 'vignette' : 'novignette'}` + `-${this.fringingEnabled ? 'fringing' : 'nofringing'}`;
      if (this._key !== key) {
        this._key = key;
